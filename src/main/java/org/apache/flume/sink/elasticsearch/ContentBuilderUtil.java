...
public class ContentBuilderUtil {
	...
	public static void addComplexField(XContentBuilder builder, String fieldName, XContentType contentType, byte[] data)
		...
		try {
			// Elasticsearch will accept JSON directly but we need to validate
			// that
			// the incoming event is JSON first. Sadly, the elasticsearch JSON
			// parser
			// is a stream parser so we need to instantiate it, parse the event
			// to
